...
public class ModelExtension {
	...
	public static final Model get(int index) {
		...
		for (int k1 = 0; k1 < m.triangleCount; k1++) {
			m.unmodifiedTriangleColor[k1] = b0.getUShort();

			if (m.triangleInfo != null) {
				m.triangleInfo[k1] = b1.getUByte();
			}

			if (m.trianglePriorities != null) {
				m.trianglePriorities[k1] = b2.getUByte();
			}

			if (m.triangleAlpha != null) {
				m.triangleAlpha[k1] = b3.getUByte();
			}

			if (m.triangleSkin != null) {
				m.triangleSkin[k1] = b4.getUByte();
			}
		}
		...
		for (int t = 0; t < m.texturedCount; t++) {
			m.textureVertexA[t] = b0.getUShort();
			m.textureVertexB[t] = b0.getUShort();
			m.textureVertexC[t] = b0.getUShort();
		}
	...
	public static final boolean unpack(int index, byte[] src) {
		...
		ModelExtensionInfo h = headers[index] = new ModelExtensionInfo();
		h.data = src;
		h.vertexCount = b.getUShort();
		h.triangleCount = b.getUShort();
		h.texturedCount = b.getUByte();

		int hasInfo = b.getUByte();
		int priority = b.getUByte();
		int hasAlpha = b.getUByte();
		int hasSkins = b.getUByte();
		int hasLabels = b.getUByte();
		int xDataLen = b.getUShort();
		int yDataLen = b.getUShort();
		int zDataLen = b.getUShort();
		int vertexIndexDataLen = b.getUShort();

...
public final class BitmapFont extends Graphics2D {
	...
	public BitmapFont(String name, Archive archive) {
		...
		for (int n = 0; n < 94; n++) {
			...
			if (type == 0) {
				for (int i = 0; i < len; i++) {
					data[n][i] = dat.getByte();
				}
			} else if (type == 1) {
				for (int x = 0; x < w; x++) {
					for (int y = 0; y < h; y++) {
						data[n][x + y * w] = dat.getByte();
					}
				}
			}
	...
	public void drawCenteredWave(String s, int x, int y, int rgb, int phase) {
		...
		for (int i = 0; i < s.length(); i++) {
			...
			if (c != 94) {
				drawChar(data[c], x + charOffsetX[c], (y + charOffsetY[c] + (int) (Math.sin((i / 2.0) + (phase / 5.0)) * 5.0)), charWidth[c], charHeight[c], rgb);
			}
			x += charSpace[c];
		}
	}
	...
	private void drawChar(byte[] data, int x, int y, int w, int h, int rgb) {
		int dstOff = x + y * targetWidth;
		int dstStep = targetWidth - w;
		int srcStep = 0;
		int srcOff = 0;
		...
		if (w > 0 && h > 0) {
			drawMask(target, data, rgb, srcOff, dstOff, w, h, dstStep, srcStep);
		}
	}

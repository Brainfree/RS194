...
public final class Buffer extends QueueLink {
	...
	private static final LinkedQueue pool1 = new LinkedQueue();
	private static final LinkedQueue pool2 = new LinkedQueue();
	private static final LinkedQueue pool3 = new LinkedQueue();
	...
	public void putOpcode(int opcode) {
		logger.log(Level.INFO, "Writing opcode: {0}", Packet.getName(opcode));
	...
	public void encode(BigInteger exponent, BigInteger modulus) {
		...
		byte[] encoded = new BigInteger(tmp).modPow(exponent, modulus).toByteArray();
		position = 0;

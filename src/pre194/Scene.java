...
public final class Scene {
	...
	public final void clearLandscape(int tileX, int tileY, int width, int height) {
		...
		for (int n = 0; n < Floor.count; n++) {
			if (Floor.instances[n].name.equalsIgnoreCase("water")) {
				water = (byte) (n + 1);
				break;
			}
		}
	...
	public final void addLoc(int locIndex, int type, SceneGraph graph, CollisionMap collision, LinkedQueue animatedLocations, int tileX, int tileY, int level, int rotation) {
		if (lowmemory) {
		...
		if (type == 22) {
			...
			if (l.animationIndex != -1) {
				animatedLocations.push(new AnimatedLocation(Animation.instance[l.animationIndex], locIndex, 2, tileX, tileY, level));
			}
		} else if (type >= 12) {
			...
			if (l.animationIndex != -1) {
				animatedLocations.push(new AnimatedLocation(Animation.instance[l.animationIndex], locIndex, 1, tileX, tileY, level));
			}
		} else if (type == 5) {
			int thickness = 16;
			...
			for (int x = -5; x < tileCountX + 5; x++) {
				for (int y = 0; y < tileCountZ; y++) {
					...
					if (dx >= 0 && dx < tileCountX) {
						...
						if (index >= 0 && index < Floor.instances.length) {
							Floor f = Floor.instances[index];
						...
						if (y >= 1 && y < tileCountZ - 1) {
							...
							if (underlayFloorIndex > 0 || overlayFloorIndex > 0) {
								...
								if (level > 0 && !lowmemory) {
									...
									if (overlayFloorIndex > 0 && overlayFloorIndex - 1 < Floor.count && !(Floor.instances[overlayFloorIndex - 1].occlude)) {
										hideUnderlay = false;
									}
								...
								if (overlayFloorIndex == 0) {
									...
									if (overlayFloorIndex >= Floor.count) {
										overlayFloorIndex = 0;
									}

									Floor f = Floor.instances[overlayFloorIndex];
